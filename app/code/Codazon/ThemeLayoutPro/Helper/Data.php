...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function canUseConfig()
    {
        if ($this->_canUseConfig === null) {
            $this->_canUseConfig = (strpos($this->_coreRegistry->registry('current_theme')->getData('code'), $this->themeNamespace) === 0);
        }
    ...
    public function getHeaderCssFile() {
        return str_replace(['https://', 'http://'], ['//', '//'], $this->_mediaUrl . $this->getHeader()->getMainCssFileRelativePath()) . '?version='. $this->getHeader()->getVersion();
    }
    
    public function getFooterCssFile() {
        return str_replace(['https://', 'http://'], ['//', '//'], $this->_mediaUrl . $this->getFooter()->getMainCssFileRelativePath()) . '?version='. $this->getFooter()->getVersion();
    }
    
    public function getMainContentCssFile() {
        return str_replace(['https://', 'http://'], ['//', '//'], $this->_mediaUrl . $this->getMainContent()->getMainCssFileRelativePath()) . '?version='. $this->getMainContent()->getVersion();
    }
